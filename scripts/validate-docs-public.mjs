...
const generated = ['index.html', '_headers', 'favicon.svg', 'api/openapi.yaml', 'api/browser-api.html', 'api/listing-flow.html', 'api/error-codes.html', 'assets/scalar.js', 'assets/scalar.css', 'assets/site.js'];
const forbidden = [
...
if (generatedExists) {
  ...
  const errorCodes = generatedText.get('api/error-codes.html');
  const browserApi = generatedText.get('api/browser-api.html');
  ...
  for (const token of ['<body class="docs-page docs-page--home">', 'id="main-content"', 'class="docs-main"', 'class="docs-shell"', 'data-menu-button', 'prefers-reduced-motion']) {
    if (!index.includes(token)) throw new Error(`[docs:public] generated layout guard is missing ${token}`);
  }
  if (!errorCodes.includes('<table>') || !browserApi.includes('data-copy-code')) throw new Error('[docs:public] generated guide enhancements are missing');
  ...
  if (!headers.includes('Content-Security-Policy:') || !headers.includes("frame-ancestors 'none'") || !headers.includes('Permissions-Policy:')) throw new Error('[docs:public] security headers are incomplete');
  const topLevel = await readdir(generatedRoot, { withFileTypes: true });
  for (const entry of topLevel) {
    if (!['index.html', '_headers', 'favicon.svg', 'api', 'assets'].includes(entry.name)) throw new Error(`[docs:public] unexpected generated entry: ${entry.name}`);
  }
