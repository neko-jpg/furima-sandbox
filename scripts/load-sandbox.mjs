...
const recordInvariant = (message, details = {}) => {
  if (stats.invariantViolations.length < 30) stats.invariantViolations.push({ message, ...details });
};

const checkState = (state, actor) => {
  if (!state || typeof state !== 'object') {
    recordInvariant('state response is not an object', { sandboxId: actor.sandboxId });
    return;
  }
  if (!Number.isInteger(state.stateVersion) || state.stateVersion < actor.lastStateVersion) {
    recordInvariant('stateVersion is not monotonic', { sandboxId: actor.sandboxId, previous: actor.lastStateVersion, received: state.stateVersion });
  ...
  for (const item of Array.isArray(state.items) ? state.items : []) {
    ...
    if (!Number.isFinite(inventory) || inventory < 0 || !Number.isFinite(reserved) || reserved < 0 || reserved > inventory) {
      recordInvariant('inventory invariant violated', { sandboxId: actor.sandboxId, itemId: item.id, inventory, reserved });
    }
  }
  for (const wallet of Array.isArray(state.wallets) ? state.wallets : []) {
    const available = Number(wallet.availableBalance ?? 0);
    const held = Number(wallet.heldBalance ?? 0);
    if (!Number.isFinite(available) || available < 0 || !Number.isFinite(held) || held < 0) {
      recordInvariant('wallet balance invariant violated', { sandboxId: actor.sandboxId, actorId: wallet.actorId, available, held });
    }
  }
};

const requestJson = async (method, path, body, kind, actor, includeInLoad = true, extraHeaders = {}) => {
  const started = nowMs();
  let response;
  let parsed;
  let raw = '';
  try {
    ...
    if (includeInLoad) {
      ...
      recordError(error instanceof Error ? error.message : 'request failed', { kind, path });
    }
    return { ok: false, status: 0, body: undefined, raw, latency, headers: new Headers() };
  }
  const latency = nowMs() - started;
  const accepted = response.status >= 200 && response.status < 300 || response.status === 304;
  if (includeInLoad) {
    stats.total += 1;
    stats.latenciesMs.push(latency);
    stats.byKind[kind] = (stats.byKind[kind] ?? 0) + 1;
    ...
    if (accepted) stats.successful += 1;
      ...
      stats.failed += 1;
      recordError(`HTTP ${response.status}`, { kind, path, body: raw.slice(0, 500) });
    }
  }
  return { ok: accepted, status: response.status, body: parsed, raw, latency, headers: response.headers };
};

const waitForOrigin = async (timeoutMs = 30_000) => {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const result = await requestJson('GET', '/api/sandbox/health?sandboxId=load-probe', undefined, 'probe', { sandboxId: 'load-probe', lastStateVersion: 0 }, false);
...
const startLocalServer = async () => {
  ...
  const serverArgs = ['run', 'start', '--', '--host', parsed.hostname, '--port', parsed.port || '3001'];
...
const setupActor = async (actor) => {
  const reset = await requestJson('POST', '/api/sandbox/reset', {
  ...
  if (!reset.ok) throw new Error(`reset failed for ${actor.sandboxId}: HTTP ${reset.status}`);
  const health = await requestJson('GET', `/api/sandbox/health?sandboxId=${encodeURIComponent(actor.sandboxId)}`, undefined, 'setup', actor, false);
  if (!health.ok || !health.body || typeof health.body.stateVersion !== 'number') throw new Error(`health failed for ${actor.sandboxId}`);
...
const runCatalog = async (actor) => {
  ...
  const result = await requestJson('GET', '/api/catalog?offset=0&limit=24&q=PC', undefined, 'catalog', actor, true, headers);
  if (result.status === 200) actor.etag = result.headers?.etag ?? actor.etag;
  return result;
};

const runHealth = async (actor) => {
  const result = await requestJson('GET', `/api/sandbox/health?sandboxId=${encodeURIComponent(actor.sandboxId)}`, undefined, 'health', actor);
  if (result.ok && result.body && typeof result.body.stateVersion === 'number' && result.body.stateVersion < actor.lastStateVersion) {
    recordInvariant('health stateVersion regressed', { sandboxId: actor.sandboxId, previous: actor.lastStateVersion, received: result.body.stateVersion });
  }
  return result;
};

const runState = async (actor) => {
  const result = await requestJson('GET', `/api/sandbox/state?sandboxId=${encodeURIComponent(actor.sandboxId)}`, undefined, 'state', actor);
...
const runPreview = async (actor) => {
  ...
  const result = await requestJson('POST', '/api/sandbox/preview', {
    sandboxId: actor.sandboxId,
  ...
  if (result.ok && result.body?.ok && result.body.data?.previewId) {
    actor.pendingPreview = { previewId: result.body.data.previewId, idempotencyKey: `${runId}-commit-${actor.index}-${sequence}` };
  }
  return result;
};
...
const runCommit = async (actor) => {
  ...
  const result = await requestJson('POST', '/api/sandbox/commit', {
    sandboxId: actor.sandboxId,
    actorId: actor.actorId,
    stateVersion: actor.lastStateVersion,
  ...
  if (typeof result.body.stateVersion === 'number') actor.lastStateVersion = result.body.stateVersion;
  actor.pendingPreview = null;
...
const runRepeatedCommit = async (actor) => {
  ...
  if (result.ok && JSON.stringify(result.body) !== repeat.expected) {
    stats.duplicateMutationViolations.push({ sandboxId: actor.sandboxId, previewId: repeat.previewId, message: 'idempotent commit response changed' });
  }
  if (result.ok && typeof result.body?.stateVersion === 'number' && result.body.stateVersion !== repeat.stateVersion) {
    stats.duplicateMutationViolations.push({ sandboxId: actor.sandboxId, previewId: repeat.previewId, message: 'idempotent commit advanced stateVersion' });
  }
  return result;
};
...
const main = async () => {
  const actors = Array.from({ length: actorCount }, (_, index) => makeActor(index));
  ...
  try {
    ...
    while (nowMs() < deadline) {
      ...
      actorCursor += 1;
      const promise = Promise.resolve(runJob(actor)).catch((error) => {
        stats.failed += 1;
        recordError(error instanceof Error ? error.message : 'job failed', { sandboxId: actor.sandboxId });
  ...
  const summary = {
    ...
    setupElapsedSeconds: Number(Math.max(0, (loadStarted - overallStarted) / 1000).toFixed(3)),
    elapsedSeconds: Number(elapsedSeconds.toFixed(3)),
    actualRps: Number((stats.total / elapsedSeconds).toFixed(2)),
    errorRate: Number(errorRate.toFixed(5)),
    latencyMs: {
      p50: Number(percentile(stats.latenciesMs, 0.50).toFixed(2)),
...
try {
  ...
  const summary = { ...stats, finishedAt: new Date().toISOString(), pass: false, fatal: error instanceof Error ? error.message : String(error) };
  await writeArtifact(summary);
  console.error(JSON.stringify({ pass: false, fatal: summary.fatal, artifact: outputPath }));
  process.exitCode = 1;
}
