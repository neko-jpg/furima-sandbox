  ...
  return (
    <div className="absolute inset-0 z-[90] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="login-prompt-title">
      <div className="w-full max-w-sm rounded-2xl border border-[var(--shop-border)] bg-[var(--shop-surface)] p-6 shadow-2xl animate-fade-in">
        ...
        <h2 id="login-prompt-title" className="mt-5 text-lg font-black text-white">ログインが必要です</h2>
        <p className="mt-2 text-sm leading-6 text-[var(--shop-muted)]">{loginPromptReason}</p>
        <p className="mt-3 rounded-lg bg-[var(--shop-surface-raised)] px-3 py-2 text-xs leading-5 text-[var(--shop-subtle)]">これはローカルappの未ログイン状態です。実際のメルカリのアカウント操作は、この画面からは行いません。</p>
        <button type="button" onClick={closeLoginPrompt} className="mt-5 w-full rounded-xl bg-[var(--shop-accent)] py-3 text-sm font-black text-white hover:bg-[var(--shop-accent-strong)]">閉じる</button>
      </div>
    </div>
  );
};
