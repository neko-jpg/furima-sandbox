...
import type { ActorProfile, FollowDirection, MercariItem, MyPagePanel, TransactionRecord, WalletLedgerType } from '../../types/mercari';
...
const readLocalDrafts = (actorId = 'seller_01'): LocalDraftRecord[] => {
...
const walletLedgerLabels: Record<WalletLedgerType, string> = {
  DEPOSIT: '入金', WITHDRAWAL: '出金', HOLD: '決済保留', CAPTURE: '決済確定', REFUND: '保留返金',
  POST_CAPTURE_REFUND: '決済後返金', SALE: '売上', SALE_REVERSAL: '売上取消', FEE: '手数料', FEE_REVERSAL: '手数料取消',
};

const walletLedgerIsDebit = (type: WalletLedgerType): boolean => ['WITHDRAWAL', 'HOLD', 'FEE', 'SALE_REVERSAL'].includes(type);
...
export const MyPageView: React.FC = () => {
  ...
  const [, setDraftRefreshTick] = React.useState(0);
  ...
  const localDrafts = readLocalDrafts(activeActor.id);
  const refreshDrafts = React.useCallback(() => setDraftRefreshTick((current) => current + 1), []);
  React.useEffect(() => {
  ...
  const openDraft = (draftId?: string) => {
    ...
    window.localStorage.setItem(scopedDraftKey(OPEN_DRAFT_STORAGE_KEY, activeActor.id), draftId);
    navigateToTab('sell');
  };
      ...
      <div className="mx-auto max-w-[1080px] px-4 pb-24 md:px-7">
        ...
        <div className={`grid gap-5 py-5 ${isDeviceFrame ? '' : 'lg:grid-cols-[300px_1fr]'}`}>
          ...
          <div className="space-y-5">
            ...
            {activeActor.role !== 'seller' && ownListings.length === 0 && <p className="rounded-xl border border-[#2b5367] bg-[#122b3a]/70 p-3 text-xs leading-5 text-[#c5eaff]">出品者の状態を確認するには、agent画面とは別の信頼済みハーネスからsellerセッションを発行してください。購入・取引データはactorごとに分離されています。</p>}
