...
import { GuidedCapturePanel } from '../../features/guided-capture/ui/GuidedCapturePanel';
import { useGuidedCaptureController, type GuidedCaptureController } from '../../features/guided-capture/ui/useGuidedCaptureController';
...
export const ListingView: React.FC = () => {
  ...
  }, [cameraFacingMode, isCameraOpen, publishGuidedCameraStream]);

  useEffect(() => {
    if (guidedCapture.state.connectionState === 'connected' && cameraStreamRef.current) {
      void publishGuidedCameraStream(cameraStreamRef.current).catch(() => undefined);
    }
  ...
  const processFiles = async (fileList: FileList | File[], source: 'camera' | 'album') => {
    ...
    try {
      ...
      for (const item of prepared.filter((candidate) => candidate.ref.status === 'ready' && candidate.previewUrl)) {
        await guidedCapture.recordMedia({ mediaId: item.ref.id, previewUrl: item.previewUrl, source: item.ref.source === 'camera' ? 'camera' : 'album', blob: item.sourceFile });
      }
  ...
  const retryMedia = async (item: ListingMediaItem) => {
    ...
    try { const result = await prepareListingMedia(item.sourceFile, item.ref.source === 'camera' ? 'camera' : 'album'); guidedCapture.removeMedia(item.ref.id); void guidedCapture.recordMedia({ mediaId: result.ref.id, previewUrl: result.previewUrl, source: result.ref.source === 'camera' ? 'camera' : 'album', blob: item.sourceFile }); setMedia((current) => current.map((candidate) => candidate.ref.id === item.ref.id ? { ref: result.ref, previewUrl: result.previewUrl, sourceFile: item.sourceFile } : candidate)); setError(null); } catch (retryError) { setError(humanizeImageError(retryError)); }
  };

  const deleteMedia = (id: string) => {
    const item = media.find((candidate) => candidate.ref.id === id); if (!item) return;
    guidedCapture.removeMedia(id);
  ...
  const confirmListing = () => {
    ...
    const guidedActive = guidedCapture.state.phase !== 'idle';
    if (guidedActive && guidedCapture.state.phase !== 'ready') {
    ...
    if (guidedActive) {
      ...
      if (!handoff.ok) { setError(handoff.message || '承認済み撮影データを下書きへ渡せませんでした。'); return; }
    }
  ...
  const applyApprovedBackground = async (blob: Blob): Promise<void> => {
    const frontId = guidedCapture.state.slots.front.mediaId;
    const currentFront = media.find((item) => item.ref.id === frontId) ?? media[0];
    if (!currentFront) throw new Error('正面画像が見つかりません。');
    const file = new File([blob], `listing-background-${Date.now()}.jpg`, { type: blob.type || 'image/jpeg' });
    const prepared = await prepareListingMedia(file, 'album');
    setMedia((current) => current.map((item) => item.ref.id === currentFront.ref.id ? { ref: prepared.ref, previewUrl: prepared.previewUrl, sourceFile: file } : item));
    guidedCapture.replaceMedia('front', { mediaId: prepared.ref.id, previewUrl: prepared.previewUrl, source: 'album', blob: file });
    guidedCapture.approveBackground(prepared.previewUrl);
    showNotice('背景編集画像を採用しました。出品確定時にこの画像を使用します。');
  };
