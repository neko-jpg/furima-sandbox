...
export const ItemDetailView: React.FC<ItemDetailViewProps> = ({ item, onClose }) => {
  ...
  const isListingUnavailable = isUnavailable || (item.listingStatus !== undefined && item.listingStatus !== 'PUBLISHED');
  ...
  const handlePurchase = () => { if (!isListingUnavailable) startPurchase(item.id); };
