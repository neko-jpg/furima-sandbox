...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!existing) {
      // A pre-read of "missing" can race another initializer. Guard creation
      // with version 0 so this request can never force an aggregate back from
      // a concurrently committed version 1 to the seed state.
