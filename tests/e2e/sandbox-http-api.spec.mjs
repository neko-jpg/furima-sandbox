...
test('sandbox reset, seed, replay, and state APIs preserve deterministic contracts', async ({ request }) => {
  ...
  const reset = await request.post('/api/sandbox/reset', { data: { id, scenarioId: 'catalog_default', seed: 'http-contract-seed-v1' } });
  ...
  const seed = await request.post('/api/sandbox/seed', { data: { id, scenarioId: 'catalog_default', seed: 'http-contract-seed-v2' } });
  ...
  const replay = await request.post('/api/sandbox/replay', {
    data: {
      ...
      actions: [
        { command: 'depositWallet', actorId: 'buyer_01', idempotencyKey: 'http-deposit-1', payload: { amount: 1 } },
        { command: 'depositWallet', actorId: 'buyer_01', idempotencyKey: 'http-deposit-1', payload: { amount: 1 } },
      ],
    },
  });
  ...
  expect(replayBody.results[0].meta.actorId).toBe('buyer_01');

  const state = await request.get(`/api/sandbox/state?id=${id}`);
  expect(state.status()).toBe(200);
  const stateBody = await state.json();
  expect(stateBody.sandboxId).toBe(id);
  expect(state.headers()['etag']).toBeTruthy();
  const notModified = await request.get(`/api/sandbox/state?id=${id}`, { headers: { 'if-none-match': state.headers()['etag'] } });
  expect(notModified.status()).toBe(304);

  const stalePut = await request.put(`/api/sandbox/state?id=${id}`, {
    headers: { 'if-match-state-version': String(Math.max(0, Number(stateBody.stateVersion) - 1)) },
    data: stateBody,
  });
